...
def create_router(cfg: Settings, db: Database, runpod: RunPod, storage: Storage, payments: CryptoPayments, limiter: RateLimiter):
    ...
    async def user_ok(event):
        ...
        if u and u["banned"]:
            await (event.answer("AccÃ¨s suspendu.") if isinstance(event,Message) else event.answer("AccÃ¨s suspendu.",show_alert=True)); return None
        return u
    ...
    async def start(m:Message, command:CommandObject):
        ...
        if u["banned"]: return await m.answer("AccÃ¨s suspendu.")
        if not u["age_verified"]: return await m.answer("ðŸ”ž Ce bot est strictement rÃ©servÃ© aux adultes. Confirmez votre Ã¢ge. Les contenus impliquant des mineurs ou l'absence de consentement sont interdits.",reply_markup=kb.age())
    ...
    async def reject_age(c:CallbackQuery): await c.message.edit_text("AccÃ¨s refusÃ© : ce service est rÃ©servÃ© aux 18+."); await c.answer()
    ...
    async def begin(c:CallbackQuery,state:FSMContext):
        ...
        if not cfg.generation_backend_ready(kind):
            return await c.answer("GÃ©nÃ©ration indisponible : configurez RunPod et R2 dans Railway.",show_alert=True)
        await state.clear(); await state.update_data(kind=kind,quality="hd" if c.data=="gen_hd" else "standard",entry=c.data); await c.message.edit_text("Choisissez le modÃ¨le :",reply_markup=kb.models()); await c.answer()
    ...
    async def outfit_start(c:CallbackQuery,state:FSMContext):
        ...
        await c.message.edit_text("ðŸ‘— Envoie la photo Ã  modifier. La personne doit Ãªtre majeure et consentante. Seuls les changements de tenue non nus sont acceptÃ©s.")
        await c.answer()
    ...
    async def outfit_upload(m:Message,state:FSMContext):
        ...
        await m.answer("DÃ©cris la nouvelle tenue souhaitÃ©e. La nuditÃ© ou la transparence explicite sur une personne rÃ©elle sera refusÃ©e.")
    ...
    async def daily_bonus(c:CallbackQuery):
        ...
        if existing:return await c.answer("Tu as dÃ©jÃ  rÃ©cupÃ©rÃ© ton bonus aujourd'hui.",show_alert=True)
        ...
        await db.credit(c.from_user.id,1); await c.answer("ðŸŽ +1 crÃ©dit ajoutÃ© !",show_alert=True)
    ...
    async def settings(c:CallbackQuery,state:FSMContext):
        ...
        elif key=="ratio" and data.get("kind")=="gen": text,markup="Choisissez la rÃ©solution :",kb.choices("resolution",list(RESOLUTIONS))
        elif key=="ratio" and data.get("kind")=="video": text,markup="Choisissez la durÃ©e :",kb.choices("duration",["2","3","4","5"])
        else:
            await state.set_state(Flow.prompt); text="Envoyez votre prompt personnalisÃ©. Les mots-clÃ©s avancÃ©s et pondÃ©rations `(mot:1.2)` sont acceptÃ©s."; markup=None
    ...
    async def prompt(m:Message,state:FSMContext):
        ...
        await m.answer(f"Confirmer :\n\n{m.text}\n\nModÃ¨le: {d['model']} Â· Style: {d['style']} Â· Niveau: {d['level']} Â· Ratio: {d['ratio']}",reply_markup=kb.confirm())

    async def process_generation(m:Message,uid:int,data:dict):
        kind=data["kind"]; cost=2 if kind=="video" else 1
        if not await db.debit(uid,cost): return await m.edit_text("CrÃ©dits insuffisants.",reply_markup=kb.shop())
        gid=await db.new_generation(uid,kind,data["prompt"],data); status=await m.edit_text("â³ GÃ©nÃ©ration en coursâ€¦ 0%")
        ...
        try:
            ...
            if kind=="video": await m.bot.send_video(uid,BufferedInputFile(raw,"creation.mp4"),caption="âœ… VidÃ©o terminÃ©e")
            else:
                preview=await storage.watermarked(raw); await m.bot.send_photo(uid,BufferedInputFile(preview,"preview.jpg"),caption="âœ… AperÃ§u filigranÃ©. L'original est conservÃ© dans votre galerie.")
            await status.edit_text("âœ… GÃ©nÃ©ration terminÃ©e.",reply_markup=kb.main())
        except Exception as e:
            task.cancel(); await db.credit(uid,cost); await db.execute("UPDATE generations SET status='failed',runpod_job_id=?,error=? WHERE id=?",(job,str(e)[:1000],gid)); log.exception("generation failed"); await status.edit_text("Ã‰chec de gÃ©nÃ©ration. Vos crÃ©dits ont Ã©tÃ© remboursÃ©s.",reply_markup=kb.main())

    async def _animate(msg:Message):
        try:
            for pct in (15,30,45,60,75,90): await asyncio.sleep(4); await msg.edit_text(f"â³ GÃ©nÃ©ration en coursâ€¦ {pct}%")
    ...
    async def gallery(c:CallbackQuery):
        ...
        i=max(0,min(i,len(items)-1)); x=items[i]; media_url=await storage.url(x["result_url"]); text=f"{x['kind'].upper()} Â· {x['created_at'][:16]}\n{x['prompt'] or 'Face swap'}\n{media_url}"
    ...
    async def shop(c:CallbackQuery): await c.message.edit_text("Achetez des crÃ©dits ou Premium :",reply_markup=kb.shop()); await c.answer()
    ...
    async def stars(c:CallbackQuery):
        ...
        await c.bot.send_invoice(c.from_user.id,"Premium mensuel" if premium else f"Pack {credits} crÃ©dits","AccÃ¨s au service 18+",payload,"XTR",[LabeledPrice(label="Total",amount=amount)]) ; await c.answer()
    ...
    async def paid(m:Message):
        ...
        await db.complete_payment(tx,p.telegram_payment_charge_id,cfg.premium_days,cfg.referral_bonus_percent); await m.answer("âœ… Paiement confirmÃ©. Votre compte a Ã©tÃ© crÃ©ditÃ©.",reply_markup=kb.main())
    ...
    async def verify(m:Message,command:CommandObject):
        ...
        if not tx or tx["status"]!="pending":return await m.answer("Paiement introuvable ou dÃ©jÃ  traitÃ©.")
        external=await payments.verify(tx)
        if not external:return await m.answer("Paiement confirmÃ© introuvable. Attendez les confirmations puis rÃ©essayez.")
        ok=await db.complete_payment(tx["id"],external,cfg.premium_days,cfg.referral_bonus_percent); await m.answer("âœ… Paiement confirmÃ© et compte crÃ©ditÃ©." if ok else "Paiement dÃ©jÃ  traitÃ©.",reply_markup=kb.main())
    ...
    async def swap(c:CallbackQuery,state:FSMContext):
        ...
        await state.clear(); await state.set_state(Flow.face); await c.message.edit_text("Envoyez une photo nette du visage. Vous devez disposer du consentement explicite de la personne reprÃ©sentÃ©e."); await c.answer()
    @r.message(Flow.face,F.photo)
    async def face(m:Message,state:FSMContext):
        f=await m.bot.get_file(m.photo[-1].file_id); buf=await m.bot.download_file(f.file_path)
        if len(buf.getvalue())>cfg.max_upload_mb*1024*1024:return await m.answer("Fichier trop volumineux.")
        ref=await storage.upload(buf.getvalue(),"jpg","image/jpeg","uploads"); url=await storage.url(ref); await state.update_data(face_url=url); await state.set_state(Flow.swap_media); await m.answer("Envoyez maintenant l'image ou la vidÃ©o cible. En l'envoyant, vous attestez avoir le consentement des personnes identifiables.")
    ...
    async def swap_media(m:Message,state:FSMContext):
        ...
        try:
            ...
            await db.execute("UPDATE generations SET status='completed',runpod_job_id=?,source_url=?,result_url=? WHERE id=?",(job,target_ref,result_ref,gid)); await (m.answer_video(BufferedInputFile(raw,"result.mp4")) if is_video else m.answer_photo(BufferedInputFile(await storage.watermarked(raw),"preview.jpg"))); await status.edit_text("âœ… Face swap terminÃ©.",reply_markup=kb.main())
        except Exception as e: await db.credit(m.from_user.id,cost); await db.execute("UPDATE generations SET status='failed',error=? WHERE id=?",(str(e)[:1000],gid)); await status.edit_text("Ã‰chec. CrÃ©dits remboursÃ©s.")
    ...
    async def admin_action(m:Message,command:CommandObject):
        ...
        try:
            ...
            await m.answer("âœ… Action appliquÃ©e.")
